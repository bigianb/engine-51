...
const float k_rand_draw_displace_amt = 1.0f;

static const Colour s_ActivateColor(0, 255, 0);
static const Colour s_DeactivateColor(255, 0, 0);
static const Colour s_MoveColor(255, 0, 255);
static const Colour s_DestroyColor(255, 255, 255);
static const Colour s_AIColor(0, 255, 255);
static const Colour s_CreateTemplateColor(255, 255, 255);

static const float s_SphereRadius = 100.0f;
...
action_create_function* actions_base::m_CreateHead = nullptr;
...
actions_base* actions_base::CreateAction(const action_types& rType, const guid& rParentGuid)
{
    ...
    if (actions_base::m_CreateHead == nullptr) {
        return nullptr;
    }

    actions_base* pNewAction = nullptr;

    action_create_function* pCurrent = actions_base::m_CreateHead;
    action_create_function* pMatched = nullptr;
...
actions_base::actions_base(guid ParentGuid, ObjectManager* om)
...
{
    /* IJB
    // Don't seem to be defined in prop_enum
...
bool actions_base::OnProperty(prop_query& rPropQuery)
{
    ...
    if (rPropQuery.IsVar("Remove")) {
        if (rPropQuery.IsRead()) {
            ...
            object_ptr<trigger_object> TrgPtr(m_ParentGuid, objectManager);
...
Vector3 actions_base::GetPositionOwner()
{
    Vector3 ZeroPos;
    ...
    object_ptr<Object> ObjectPtr(m_ParentGuid, objectManager);
