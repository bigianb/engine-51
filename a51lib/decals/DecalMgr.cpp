...
static const float  kBulletHoleCoplanarDistCheck = 2.0f;
static const float  kCoplanarDistanceCheck = 2.5f;
static const float  kNearZBias = 0.01f;
static const Radian kIncomingAngleMinimum = R_20;
static const float  kEpsilon = 0.005f;
...
static Matrix4 s_IdentityL2W;

//==============================================================================
// Helper functions
//==============================================================================

inline Vector3 GetTriNormal(const Vector3& P0,
                            const Vector3& P1,
                            const Vector3& P2,
                            bool          bNormalize)
{
...
void decal_mgr::registration_info::Kill()
{
...
int decal_mgr::registration_info::GetAllocSize(int nVerts)
{
    int AllocSize = 0;
    AllocSize += nVerts * sizeof(position_data);
    AllocSize += nVerts * sizeof(uv_data);
    AllocSize += nVerts * sizeof(uint32_t);
    if (m_Flags & decal_definition::DECAL_FLAG_FADE_OUT) {
        AllocSize += nVerts * sizeof(float);
    }
    return AllocSize;
}
...
void decal_mgr::registration_info::AllocVertList(int nVerts)
{
    int AllocSize = GetAllocSize(m_nVertsAllocated + nVerts);
    if (m_nVertsAllocated == 0) {
        // easy case, just alloc the new verts
        m_nVertsAllocated = nVerts;
        uint8_t* pAllocAddress = (uint8_t*)malloc(AllocSize);
        m_pPositions = (position_data*)pAllocAddress;
        pAllocAddress += m_nVertsAllocated * sizeof(position_data);
        m_pUVs = (uv_data*)pAllocAddress;
        pAllocAddress += m_nVertsAllocated * sizeof(uv_data);
        m_pColors = (uint32_t*)pAllocAddress;
        pAllocAddress += m_nVertsAllocated * sizeof(uint32_t);
        if (m_Flags & decal_definition::DECAL_FLAG_FADE_OUT) {
            m_pElapsedTimes = (float*)pAllocAddress;
            pAllocAddress += m_nVertsAllocated * sizeof(float);
        }
        assert(pAllocAddress == ((uint8_t*)m_pPositions) + AllocSize);
    ...
    , m_pStaticData(nullptr)
{
    ...
    m_TriangleTemplateUV[0].U = (int16_t)((+0.500f) * 4096.0f);
    m_TriangleTemplateUV[0].V = (int16_t)((+1.500f) * 4096.0f);
    m_TriangleTemplateUV[1].U = (int16_t)((+1.366f) * 4096.0f);
    m_TriangleTemplateUV[1].V = (int16_t)((+0.000f) * 4096.0f);
    m_TriangleTemplateUV[2].U = (int16_t)((-0.366f) * 4096.0f);
    m_TriangleTemplateUV[2].V = (int16_t)((+0.000f) * 4096.0f);
}
...
void decal_mgr::ClearDynamicQueue()
{
    int i;
    m_DynamicQueueAddPos = 0;
    m_DynamicQueueReadPos = 0;
...
void decal_mgr::ResetDynamicDecals()
{
    int i;
                                       ...
                                       Radian                  Roll)
{
    ...
    int nVerts = CalcDecalVerts(Def.m_Flags,
                                Collision.Point,
                                 ...
                                 const Vector3*          pTriPos)
{
    ...
    assert(Def.m_Flags & decal_definition::DECAL_FLAG_USE_TRI); //, "Bulletholes must have DECAL_FLAG_USE_TRI");
    ...
    float Radius = x_frand(Def.m_MinSize.x, Def.m_MaxSize.x);
    AxisA *= Radius;
    AxisB *= Radius;
    ...
    //
    {
        int iDecalStart = GetDecalStart(Def.m_Handle, 3);
        if (iDecalStart != -1) {
            ...
            uint32_t*      pColor = &RegInfo.m_pColors[iDecalStart];
            float*         pTime = (RegInfo.m_Flags & decal_definition::DECAL_FLAG_FADE_OUT) ? &RegInfo.m_pElapsedTimes[iDecalStart] : nullptr;

            uint32_t Color = RegInfo.m_Color.getRGBA();
...
xhandle decal_mgr::RegisterDefinition(decal_definition& Def)
{
    ...
    if (Def.m_Handle.IsNonNull()) {
        assert(false); //, "Attempt to register a decal multiple times!?!?");
        registration_info& RegInfo = m_RegisteredDefs(Def.m_Handle);
        RegInfo.Kill();
    ...
    // No paths allowed!
    char* p = &Def.m_BitmapName[strlen(Def.m_BitmapName)];
    ...
    int HWBufferSize = render::GetHardwareBufferSize();
    int nVertsToAlloc = 0;
...
int decal_mgr::CalcNoClipDecal(int            Flags,
                               const Vector3& Point,
                               const Vector3& Normal,
                               const Vector2& Size,
                               Radian         Roll,
                               ...
                               Matrix4&       L2W)
{
    ...
    if (Flags & decal_definition::DECAL_FLAG_USE_TRI) {
        ...
        float VerticalRadius = 0.5f * Size.y;
    ...
    for (i = 0; i < nVerts; i++) {
        if (abs(DecalPlane.Distance(Collisions[i])) > kCoplanarDistanceCheck) {
            return 0;
        }
    }
...
int decal_mgr::CalcProjectedDecal(const Vector3& Point,
                                  ...
                                  Matrix4&       L2W)
{
    ...
    plane   SurfacePlane;
    Radian  theta = acos(NegIncomingRay.Dot(SurfaceNormal));
    ...
    if (theta >= (R_90 - kIncomingAngleMinimum)) {
        ...
        ProjectionRay = Perpendicular * sin(kIncomingAngleMinimum) +
                        Parallel * cos(kIncomingAngleMinimum);
    ...
    L2W.Setup(Vector3(1.0f, 1.0f, 1.0f),
              ...
              Point);
    float RayDotNormal = ProjectionRay.Dot(SurfacePlane.Normal);
    float AbsDot = abs(RayDotNormal);
    for (i = 0; i < 4; i++) {
    ...
    if (0) {
        ...
        for (int iVert = 0; iVert < 18; iVert++) {
        ...
        working_data* pWorkingData = (working_data*)malloc(sizeof(working_data));
        ...
        int nVerts = pWorkingData->nFinalVerts;
        if (nVerts) {
...
/*
    ...
    Surface* pSurface = g_PlaySurfaceMgr.GetNextSurface();
    while (pSurface != nullptr) {
        render::hgeom_inst hGeomInst = pSurface->RenderInst;
        const RigidGeom*  pRigidGeom = (const RigidGeom*)render::GetGeom(hGeomInst);

        if (pRigidGeom && (pRigidGeom->collision.numHighClusters != 0)) {
...
static int ClipNegXVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
    bool bNextIn = pSrc[NextVert].GetX() >= (-1.0f - kEpsilon);
    int   NDst = 0;
    ...
    for (int i = 0; i < NSrcVerts; i++) {
        ...
        if (bCurrIn != bNextIn) {
            ...
            float T = (-1.0f - pSrc[CurrVert].GetX()) / Denom;
...
static int ClipPosXVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
...
static int ClipNegYVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
    bool bNextIn = pSrc[NextVert].GetY() >= (-1.0f - kEpsilon);
    int   NDst = 0;
    ...
    for (int i = 0; i < NSrcVerts; i++) {
        ...
        if (bCurrIn != bNextIn) {
            ...
            float T = (-1.0f - pSrc[CurrVert].GetY()) / Denom;
            pDst[NDst++] = pSrc[CurrVert] + T * V;
        }
    }
...
static int ClipPosYVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
...
static int ClipNegZVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
    bool bNextIn = pSrc[NextVert].GetZ() >= (-1.0f - kEpsilon);
    int   NDst = 0;
    ...
    for (int i = 0; i < NSrcVerts; i++) {
        ...
        if (bCurrIn != bNextIn) {
            ...
            float T = (-1.0f - pSrc[CurrVert].GetZ()) / Denom;
            pDst[NDst++] = pSrc[CurrVert] + T * V;
        }
    }
...
static int ClipPosZVerts(Vector3* pDst, const Vector3* pSrc, int NSrcVerts)
{
    int   CurrVert;
    bool bCurrIn;
    int   NextVert = NSrcVerts - 1;
...
void decal_mgr::AddGeometryToDecal(const RigidGeom* pRigidGeom,
                                   ...
                                   working_data&     WD)
{
    ...
    for (int iCluster = 0; iCluster < pRigidGeom->collision.numHighClusters; iCluster++) {
        for (int iTri = 0; iTri < pRigidGeom->collision.highClusters[iCluster].nTris; iTri++) {
            int Key = (iCluster << 16) | iTri;
            ...
            uint32_t ClipBits = 0;
            if (ClipVerts[Buff][0].GetX() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_X << 0);
            }
            if (ClipVerts[Buff][0].GetX() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_X << 0);
            }
            if (ClipVerts[Buff][0].GetY() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Y << 0);
            }
            if (ClipVerts[Buff][0].GetY() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Y << 0);
            }
            if (ClipVerts[Buff][0].GetZ() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Z << 0);
            }
            if (ClipVerts[Buff][0].GetZ() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Z << 0);
            }
            if (ClipVerts[Buff][1].GetX() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_X << 6);
            }
            if (ClipVerts[Buff][1].GetX() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_X << 6);
            }
            if (ClipVerts[Buff][1].GetY() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Y << 6);
            }
            if (ClipVerts[Buff][1].GetY() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Y << 6);
            }
            if (ClipVerts[Buff][1].GetZ() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Z << 6);
            }
            if (ClipVerts[Buff][1].GetZ() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Z << 6);
            }
            if (ClipVerts[Buff][2].GetX() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_X << 12);
            }
            if (ClipVerts[Buff][2].GetX() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_X << 12);
            }
            if (ClipVerts[Buff][2].GetY() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Y << 12);
            }
            if (ClipVerts[Buff][2].GetY() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Y << 12);
            }
            if (ClipVerts[Buff][2].GetZ() < -1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_NEG_Z << 12);
            }
            if (ClipVerts[Buff][2].GetZ() > 1.0f) {
                ClipBits |= (CLIP_DECAL_VERT_POS_Z << 12);
            }

            // trivial rejection
            if ((ClipBits & CLIP_DECAL_TRI_NEG_X) == CLIP_DECAL_TRI_NEG_X) {
                continue;
            }
            if ((ClipBits & CLIP_DECAL_TRI_POS_X) == CLIP_DECAL_TRI_POS_X) {
                continue;
            }
            if ((ClipBits & CLIP_DECAL_TRI_NEG_Y) == CLIP_DECAL_TRI_NEG_Y) {
                continue;
            }
            if ((ClipBits & CLIP_DECAL_TRI_POS_Y) == CLIP_DECAL_TRI_POS_Y) {
                continue;
            }
            if ((ClipBits & CLIP_DECAL_TRI_NEG_Z) == CLIP_DECAL_TRI_NEG_Z) {
                continue;
            }
            if ((ClipBits & CLIP_DECAL_TRI_POS_Z) == CLIP_DECAL_TRI_POS_Z) {
                continue;
            }
...
void decal_mgr::CreateIndexedVertPool(working_data& WD)
{
    ...
    for (i = 0; i < WD.nVerts; i++) {
        ...
        for (j = 0; j < WD.nIndexedVerts; j++) {
            if ((abs(pSrcVerts[i].GetX() - pDstVerts[j].GetX()) < kEpsilon) &&
                (abs(pSrcVerts[i].GetY() - pDstVerts[j].GetY()) < kEpsilon) &&
                (abs(pSrcVerts[i].GetZ() - pDstVerts[j].GetZ()) < kEpsilon)) {
                break;
            }
        }
...
{
    int16_t VertIndices[working_data::MAX_WORKING_VERTS];
    int i, j, k;
    ...
    for (i = 0; i < WD.nVerts - 2; i++) {
        ...
        if ((WD.VertFlags[i] & working_data::FLAG_POLY_START) &&
            ...
            if (nNewPolyVerts != (PolyEnd - PolyStart)) {
                ...
                int NewPolyEnd = PolyStart + nNewPolyVerts;

                memmove(&WD.RemapIndices[NewPolyEnd],
                          &WD.RemapIndices[PolyEnd],
                          sizeof(int16_t) * (WD.nVerts - PolyEnd));
                memmove(&WD.VertFlags[NewPolyEnd],
                          &WD.VertFlags[PolyEnd],
                          sizeof(uint16_t) * (WD.nVerts - PolyEnd));
...
bool decal_mgr::CollectCoplanarEdges(working_data& WD,
                                      plane&        PolyPlane)
        ...
        if ((WD.VertFlags[i] & working_data::FLAG_POLY_START) &&
            ...
            for (j = PolyEnd; j < WD.nVerts - 2; j++) {
                if ((WD.VertFlags[j] & working_data::FLAG_POLY_START) &&
                    ...
                    if (abs(TestPlane.D - PolyPlane.D) > kEpsilon) {
                        continue;
                    }

                    float Dot = TestPlane.Normal.Dot(PolyPlane.Normal);
...
int DecalEdgeSortFn(const void* pA, const void* pB)
{
    decal_mgr::decal_edge* pEdgeA = (decal_mgr::decal_edge*)pA;
    decal_mgr::decal_edge* pEdgeB = (decal_mgr::decal_edge*)pB;
    if (pEdgeA->P0 < pEdgeB->P0) {
        return -1;
    } else if (pEdgeA->P0 > pEdgeB->P0) {
        return 1;
    } else if (pEdgeA->P1 < pEdgeB->P1) {
        return -1;
    } else if (pEdgeA->P1 > pEdgeB->P1) {
        return 1;
    }
...
void decal_mgr::RemoveDuplicateEdges(working_data& WD)
{
    ...
    WD.nEdges = nNewEdges;
    memcpy(WD.EdgeList, NewEdgeList, sizeof(decal_edge) * nNewEdges);
}

//==============================================================================

bool decal_mgr::GetCoplanarPolyVerts(working_data& WD)
{
    int i, j;
    ...
    for (i = 0; i < WD.nEdges; i++) {
        ...
        while (!CompleteLoop) {
            if (++SafetyCount > 500) {
                assert(false); //, "Internal decal error.");
                return false;
            }
...
bool decal_mgr::IsCollinear(const Vector3& StartPoint,
                            ...
                            const plane&  PolyPlane)
{
    ...
    int i;
    int nVerts = WD.nCoplanarPolyVerts;
    for (i = 0; i < nVerts; i++) {
        int P0 = WD.CoplanarPolyVerts[i];
        int P1 = WD.CoplanarPolyVerts[(i + 1) % nVerts];
        int P2 = WD.CoplanarPolyVerts[(i + 2) % nVerts];
    ...
    if (PolyPlane.Normal.Dot(Normal) < 0.0f) {
        for (i = 0; i < nVerts / 2; i++) {
            int16_t Temp = WD.CoplanarPolyVerts[i];
...
void decal_mgr::BuildConvexStrip(working_data& WD)
{
    ...
    // useful ptrs
    const int16_t*     pVertIndices = WD.CoplanarPolyVerts;
    ...
    int i;
    int Index = 1;
    int nPolyVerts = WD.nCoplanarPolyVerts;
...
bool decal_mgr::TriContainsVert(working_data& WD,
                                 int           P0,
                                 int           P1,
                                 int           P2,
                                 int           PointToTest)
...
bool decal_mgr::IsAnEar(working_data&    WorkingData,
                         ...
                         int              VertToTest)
{
    if (FirstConcave == -1) {
        return true;
    } else if (!VertPool[VertToTest].bConcave) {
        int i = FirstConcave;
        int Prev = VertPool[VertToTest].iPrev;
        int Next = VertPool[VertToTest].iNext;
...
bool decal_mgr::IsConvex(working_data&    WD,
                          triangulate_link VertPool[MAX_VERTS_PER_DECAL],
                          int              VertToTest,
                          const plane&     PolyPlane)
{
    int     P0 = WD.CoplanarPolyVerts[VertPool[VertToTest].iPrev];
    int     P1 = WD.CoplanarPolyVerts[VertToTest];
    int     P2 = WD.CoplanarPolyVerts[VertPool[VertToTest].iNext];
    Vector3 TriN = GetTriNormal(WD.IndexedVertPool[P0],
                                WD.IndexedVertPool[P1],
                                WD.IndexedVertPool[P2],
                                false);
    float   Dot = TriN.Dot(PolyPlane.Normal);
                            ...
                            const plane&  PolyPlane)
{
    ...
    while (i != 0) {
        if (++SafetyCount > 500) {
            assert(false); //, "Internal decal error");
            break;
        } else if (IsAnEar(WD, VertPool, ConcaveStart, VertPool[i].iPrev) &&
                   (nPolyVerts >= 3)) {
            // see if there are two shared verts that we can continue a strip with
            bool bNewStrip = true;
            int   P0 = WD.CoplanarPolyVerts[VertPool[i].iPrev];
            int   P1 = WD.CoplanarPolyVerts[i];
            int   P2 = WD.CoplanarPolyVerts[VertPool[VertPool[i].iPrev].iPrev];
            int   P3 = -1;
            if (((P0 == PrevStrip0) || (P1 == PrevStrip0)) &&
                ((P0 == PrevStrip1) || (P1 == PrevStrip1))) {
                bNewStrip = false;
                P3 = P2;
            } else if (((P0 == PrevStrip0) || (P2 == PrevStrip0)) &&
                       ((P0 == PrevStrip1) || (P2 == PrevStrip1))) {
                bNewStrip = false;
                P3 = P1;
            } else if (((P1 == PrevStrip0) || (P2 == PrevStrip0)) &&
...
void decal_mgr::CombineCoplanarPolys(working_data& WD)
{
    ...
    while (CollectCoplanarEdges(WD, PolyPlane)) {
        // sort the edges
        qsort(WD.EdgeList, WD.nEdges, sizeof(decal_edge), DecalEdgeSortFn);
        ...
        while (GetCoplanarPolyVerts(WD)) {
            int16_t NewPolyVertIndices[working_data::MAX_WORKING_VERTS];
            int nNewPolyVerts = 0;
            int i;

            // remove collinear verts from this poly
            for (i = 0; i < WD.nCoplanarPolyVerts; i++) {
                int StartPoint = WD.CoplanarPolyVerts[i];
                int MidPoint = WD.CoplanarPolyVerts[(i + 1) % WD.nCoplanarPolyVerts];
                int EndPoint = WD.CoplanarPolyVerts[(i + 2) % WD.nCoplanarPolyVerts];
            ...
            if (nNewPolyVerts >= 3) {
                ...
                memcpy(WD.CoplanarPolyVerts, NewPolyVertIndices, nNewPolyVerts * sizeof(int16_t));
...
int decal_mgr::CalcDecalVerts(int            Flags,
                              const Vector3& Point,
                              const Vector3& SurfaceNormal,
                              const Vector3& NegIncomingRay,
                              const Vector2& Size,
...
int decal_mgr::GetDecalStart(xhandle RegInfoHandle, int nVerts)
{
    ...
    while (!bAlloced) {
        if (RegInfo.m_End >= RegInfo.m_Start) {
            ...
            int BufferEnd = RegInfo.m_Blank +
                            HWBufferSize -
                            (RegInfo.m_Blank % HWBufferSize);

            int nFreeInHWBuff = BufferEnd - RegInfo.m_Blank;
            int nFreeInNextHWBuff = (BufferEnd == RegInfo.m_nVertsAllocated) ? 0 : HWBufferSize;
            if (nFreeInHWBuff > nVerts) {
            ...
            } else if (nFreeInNextHWBuff > nVerts) {
                ...
                int nVertsAvailable = std::min(RegInfo.m_Start, HWBufferSize);
            ...
            int BufferEnd = RegInfo.m_End +
                            HWBufferSize -
                            (RegInfo.m_End % HWBufferSize);

            int nFreeInHWBuff = (BufferEnd > RegInfo.m_Start) ? RegInfo.m_Start - RegInfo.m_End : BufferEnd - RegInfo.m_End;
            int nFreeInNextHWBuff = (BufferEnd + HWBufferSize > RegInfo.m_Start) ? RegInfo.m_Start - BufferEnd : HWBufferSize;
...
void decal_mgr::AddDecal(xhandle        RegInfoHandle,
                         ...
                         const Matrix4& L2W)
{
    int DecalStart = GetDecalStart(RegInfoHandle, nVerts);
    if (DecalStart != -1) {
        registration_info& RegInfo = m_RegisteredDefs(RegInfoHandle);
        ...
        for (i = 0; i < nVerts; i++) {
            ...
            RegInfo.m_pUVs[DecalStart + i].U = (int16_t)(DecalVerts[i].UV.x * 4096.0f);
            RegInfo.m_pUVs[DecalStart + i].V = (int16_t)(DecalVerts[i].UV.y * 4096.0f);
...
void decal_mgr::RenderVerts(int nVerts, position_data* pPos, uv_data* pUV, uint32_t* pColor)
{
    /*
#ifdef TARGET_PS2
    render::RenderRawStrips(nVerts,
                            s_IdentityL2W,
                            (vector4*)((uint32_t)pPos & 0x0fffffff),     // because we've used uncached access
                            (int16_t*)((uint32_t)pUV & 0x0fffffff),          // because we've used uncached access
                            (uint32_t*)((uint32_t)pColor & 0x0fffffff)); // because we've used uncached access
    ...
    render::RenderRawStrips(nVerts,
                            ...
                            pColor);
#else
    static const float ItoFScale = 1.0f / 4096.0f;
    ...
    for (iVert = 0; iVert < nVerts; iVert++) {
        ...
        if (WindingCW) {
            ...
            draw_UV((float)pUV[iVert - 2].U * ItoFScale, (float)pUV[iVert - 2].V * ItoFScale);
            draw_Vertex(pPos[iVert - 2].Pos);

            draw_Color(pColor[iVert - 1]);
            draw_UV((float)pUV[iVert - 1].U * ItoFScale, (float)pUV[iVert - 1].V * ItoFScale);
            draw_Vertex(pPos[iVert - 1].Pos);

            draw_Color(pColor[iVert - 0]);
            draw_UV((float)pUV[iVert - 0].U * ItoFScale, (float)pUV[iVert - 0].V * ItoFScale);
...
void decal_mgr::RenderDynamicDecals(registration_info& RegInfo)
{
    ...
    if (RegInfo.m_End > RegInfo.m_Start) {
        ...
        int VertStart = RegInfo.m_Start / HWBufferSize;
        VertStart *= HWBufferSize;

        int nVerts = RegInfo.m_End - VertStart;
        if (nVerts) {
            RenderVerts(nVerts,
...
void decal_mgr::OnRender()
{
    ...
    if (1) {
        float NearZOffset = OldNear * kNearZBias;
    ...
    for (int i = 0; i < m_RegisteredDefs.GetCount(); i++) {
        registration_info& RegInfo = m_RegisteredDefs[i];
        ...
        uint32_t DrawFlags = DRAW_USE_ALPHA | DRAW_NO_ZWRITE | DRAW_TEXTURED | DRAW_UV_CLAMP;
        ...
        default:
        case decal_definition::DECAL_BLEND_NORMAL:
            BlendMode = render::BLEND_MODE_NORMAL;
            break;
        case decal_definition::DECAL_BLEND_ADD:
            BlendMode = render::BLEND_MODE_ADDITIVE;
            break;
        case decal_definition::DECAL_BLEND_SUBTRACT:
            BlendMode = render::BLEND_MODE_SUBTRACTIVE;
            break;
        case decal_definition::DECAL_BLEND_INTENSITY:
            BlendMode = render::BLEND_MODE_INTENSITY;
            break;
        }
...
void decal_mgr::UpdateAlphaFade(float DeltaTime, float FadeTime, int nVerts, uint32_t* pColor, float* pTimeElapsed)
{
    int i;
    for (i = 0; i < nVerts; i++) {
        pTimeElapsed[i] += DeltaTime;
        float T = 1.0f - (pTimeElapsed[i] / FadeTime);
        T = std::min(T, 1.0f);
        T = std::max(T, 0.0f);
#ifdef TARGET_PS2
        pColor[i] &= 0x00ffffff;
...
void decal_mgr::OnUpdate(float DeltaTime)
{
    ...
    if (m_DynamicQueue[m_DynamicQueueReadPos].Valid) {
        ...
        // calculate the decal verts
        int nVerts = CalcDecalVerts(QElement.Flags,
                                    QElement.Point,
                                    QElement.Normal,
                                    QElement.NegRay,
                                    QElement.Size,
