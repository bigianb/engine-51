...
public:
    ...
    int      GetVMeshIndex(const char* pName) const { return -1; /* TODO */ }
    uint64_t GetLODMask(uint32_t VMeshMask, uint16_t ScreenSize) const;
    ...
    bool GetPropertyInteger(const PropertySection* pSection, const char* pName, int* pValue) const;
    bool GetPropertyAngle(const PropertySection* pSection, const char* pName, Radian* pValue) const;
    bool GetPropertyString(const PropertySection* pSection, const char* pName, char* pValue, int nChars) const;
...
public:
    ...
    VirtualMaterial* virtualMaterials;
