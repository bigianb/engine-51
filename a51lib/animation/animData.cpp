...
uint32_t AnimKeyStream::GetFlags() const
{
    return (Offset >> STREAM_FLG_SHIFT) & STREAM_FLG_MASK;
}

void AnimKeyStream::GetInterpKey(uint8_t* pData, int nFrames, int iFrame, float T, AnimKey& Key) const
{
    AnimKey K0;
    AnimKey K1;

    grabKey(pData, nFrames, iFrame + 0, K0);
    grabKey(pData, nFrames, iFrame + 1, K1);
