...
const float k_DistToIncreaseYaw = 50.0f;

Geom::BoneMask loco::s_ZeroBoneMasks;
Geom::BoneMask loco::s_OneBoneMasks;
...
void loco::move_style_info::InitDefaults(move_style_info_default& Default)
{
    ...
    if (Default.m_hAnimGroup.getPointer()) {
        ...
        memcpy(m_iAnims, Default.m_iAnims, sizeof(m_iAnims));
    }
}
...
bool loco_play_anim::OnExit(void)
{
...
void loco_play_anim::OnAdvance(float DeltaTime)
{
    ...
    if (m_Flags & loco::ANIM_FLAG_PLAY_TYPE_CYCLIC) {
        // Finished cyclic?
        if ((m_Base.m_Player.IsAtEnd()) && (m_Base.m_Player.GetCycle() >= (int)m_PlayTime)) {
            m_bComplete = true;
        }
    }
...
bool loco_play_anim::PlayAnim(const AnimGroup::handle& hAnimGroup, int iAnim, float BlendTime, uint32_t Flags, float PlayTime)
{
...
void loco_idle::OnAdvance(float DeltaTime)
{
    // Update timer
    m_Timer += DeltaTime;

    // Lookup useful info
    bool bBlending = m_Base.m_Player.IsBlending();
    bool bIsAtEnd = m_Base.m_Player.IsAtEnd();
...
*/
    ...
    if (!bBlending) {
        // Switch to new anim?
        float BlendTime = Info.m_IdleBlendTime;
        ...
        if ((m_Base.m_Motion == loco::MOTION_NULL) && (m_Base.m_pPrev)) {
            ...
            if (m_Base.m_pPrev->m_State == loco::STATE_PLAY_ANIM) {
                ...
                BlendTime = std::max(Info.m_IdleBlendTime, Info.m_MoveBlendTime);
            }
        }
        ...
        bool bAccumYawMotion = true;
        bool bRemoveYawMotion = true;
        bool bAccumHorizMotion = true;
        bool bRemoveHorizMotion = true;
        bool bLooping = true;
        ...
        // TurnLeft180?
        if ((H > Info.m_IdleTurn180DeltaYawMax) && (Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_LEFT_180] != -1)) {
            if (((m_Base.m_Motion == loco::MOTION_IDLE_TURN_180) && (bIsAtEnd)) || (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180)) {
                iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_LEFT_180];
                m_Base.m_Motion = loco::MOTION_IDLE_TURN_180;
            }
        } else
            // TurnRight180?
            if ((H < Info.m_IdleTurn180DeltaYawMin) && (Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_RIGHT_180] != -1)) {
                if (((m_Base.m_Motion == loco::MOTION_IDLE_TURN_180) && (bIsAtEnd)) || (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180)) {
                ...
                // Turn180?
                if (((H > Info.m_IdleTurn180DeltaYawMax) || (H < Info.m_IdleTurn180DeltaYawMin)) && (Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_180] != -1)) {
                    if (((m_Base.m_Motion == loco::MOTION_IDLE_TURN_180) && (bIsAtEnd)) || (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180)) {
                    ...
                    // Turn right?
                    if ((H < IdleTurnDeltaYawMin) && (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180) && (Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_RIGHT] != -1)) {
                        if (((m_Base.m_Motion == loco::MOTION_IDLE_TURN_RIGHT) && (bIsAtEnd)) || (m_Base.m_Motion != loco::MOTION_IDLE_TURN_RIGHT)) {
                            iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_RIGHT];
                            m_Base.m_Motion = loco::MOTION_IDLE_TURN_RIGHT;
                        }
                    } else
                        // Turn left?
                        if ((H > IdleTurnDeltaYawMax) && (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180) && (Info.m_iAnims[loco::MOVE_STYLE_ANIM_IDLE_TURN_LEFT] != -1)) {
                            if (((m_Base.m_Motion == loco::MOTION_IDLE_TURN_LEFT) && (bIsAtEnd)) || (m_Base.m_Motion != loco::MOTION_IDLE_TURN_LEFT)) {
                            ...
                            bool bPlayIdle = bIsAtEnd;

                            // Play idle animation if not currently playing any
                            bPlayIdle |= (m_Base.m_Motion != loco::MOTION_IDLE) && (m_Base.m_Motion != loco::MOTION_IDLE_TURN_LEFT) && (m_Base.m_Motion != loco::MOTION_IDLE_TURN_RIGHT) && (m_Base.m_Motion != loco::MOTION_IDLE_TURN_180);
...
void loco_move::OnAdvance(float DeltaTime)
{
    ...
    if (m_Base.m_bExactMove) {
        ...
        if (m_Base.m_AnimState == loco::STATE_MOVE) {
            ...
            float ArriveDistSqr = m_Base.ComputeExactArriveDistSqr();
    ...
    float BlendTime = m_bFirstTime ? Info.m_IdleBlendTime : Info.m_MoveBlendTime;

    // If playing the same motion or it's the first time in move, limit the turning
    if ((m_bFirstTime) || (m_Base.m_Motion == Motion) || (abs(DeltaYaw) < R_45)) {
        ...
        if (DeltaYaw > MaxDeltaYaw) {
            DeltaYaw = MaxDeltaYaw;
        } else if (DeltaYaw < -MaxDeltaYaw) {
            DeltaYaw = -MaxDeltaYaw;
        }

        // Compute distance D from moveat
        float DSqr = (m_Base.GetMoveAt() - m_Base.GetPosition()).LengthSquared();
        float D = 0.0f;
        if (DSqr > 0.0001f) {
            D = sqrt(DSqr);
        }
    ...
    int      iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_MOVE_FRONT];
    uint32_t Flags = 0;
    switch (Motion) {
    case loco::MOTION_FRONT:
        iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_MOVE_FRONT];
        break;
    case loco::MOTION_LEFT:
        iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_MOVE_LEFT];
        break;
    case loco::MOTION_RIGHT:
        iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_MOVE_RIGHT];
        break;
    case loco::MOTION_BACK:
        iAnim = Info.m_iAnims[loco::MOVE_STYLE_ANIM_MOVE_BACK];
        break;
    default:
        break;
    }
    ...
    if ((!m_Base.m_Player.IsBlending()) &&
        ...
        float Frame = m_Base.m_Player.GetCurrAnim().GetFrameParametric();
...
// using ArriveDistSqr.
bool loco::HasArrivedAtPosition(const Vector3& Pos, float ArriveDistSqr)
{
    ...
    if (m_bGhostMode) {
        ...
        float SpeedSqr = m_DeltaPos.LengthSquared();
    ...
    // Check Y first
    if (abs(Delta.GetY()) > 400) {
        return false;
    }

    // Now just check X and Z
    float XZDistSqr = x_sqr(Delta.GetX()) + x_sqr(Delta.GetZ());
...
float loco::ComputeExactArriveDistSqr(void)
{
    ...
    float u = m_Player.GetMovementSpeed();
    float t = std::max(m_MoveStyleInfo.m_IdleBlendTime, m_MoveStyleInfo.m_MoveBlendTime);
    float ArriveDist = 0.5f * u * t;
    ...
    //ArriveDist *= 1.5f;
    ArriveDist = std::max(ArriveDist, 10.0f);

    // Returns distance squared
    float ArriveDistSqr = x_sqr(ArriveDist);
...
const char* loco::GetMoveStyleName(int MoveStyle)
{
    // Which state?
    switch (MoveStyle) {
    default:
...
const char* loco::GetMoveStyleHeader(int MoveStyle)
{
    // Which state?
    switch (MoveStyle) {
    default:
    ...
    case loco::MOVE_STYLE_CHARGE:
        return "CHARGE\\";
    case loco::MOVE_STYLE_CHARGE_FAST:
        return "CHARGE_FAST\\";
    }
}
...
const char* loco::GetMoveStyleAnimName(int MoveStyleAnim)
{
    ...
    case MOVE_STYLE_ANIM_IDLE:
        return "IDLE";
    case MOVE_STYLE_ANIM_IDLE_TURN_LEFT:
        return "IDLE_TURN_LEFT";
    case MOVE_STYLE_ANIM_IDLE_TURN_RIGHT:
        return "IDLE_TURN_RIGHT";
    case MOVE_STYLE_ANIM_IDLE_TURN_180:
        return "IDLE_TURN_180";
    case MOVE_STYLE_ANIM_IDLE_TURN_LEFT_180:
        return "IDLE_TURN_LEFT_180";
    case MOVE_STYLE_ANIM_IDLE_TURN_RIGHT_180:
        return "IDLE_TURN_RIGHT_180";
    case MOVE_STYLE_ANIM_IDLE_FIDGET:
        return "IDLE_FIDGET";
    case MOVE_STYLE_ANIM_MOVE_FRONT:
        return "MOVE_FRONT";
    case MOVE_STYLE_ANIM_MOVE_LEFT:
        return "MOVE_LEFT";
    case MOVE_STYLE_ANIM_MOVE_BACK:
        return "MOVE_BACK";
    case MOVE_STYLE_ANIM_MOVE_RIGHT:
        return "MOVE_RIGHT";
    }
}
...
const char* loco::GetMoveStyleEnum(void)
{
    ...
    char* pDest = s_Enum;
    for (int i = 0; i < MOVE_STYLE_COUNT; i++) {
        // Lookup state name
...
loco::move_style loco::GetMoveStyleByName(const char* pName)
{
    ...
    for (int i = 0; i < MOVE_STYLE_COUNT; i++) {
        // Found?
        if (strcasecmp(pName, GetMoveStyleName(i)) == 0) {
...
loco::loco(ObjectManager* pObjectManager, collision_mgr* collisionManager)
    ...
    , // true if character has been stuck for a while
    m_bAllowFrontMotion(true)
    , // Controls if character can play MOVE_FRONT animations
    m_bAllowLeftMotion(true)
    , // Controls if character can play MOVE_LEFT animations
    m_bAllowBackMotion(true)
    , // Controls if character can play MOVE_BACK animations
    m_bAllowRightMotion(true)
    , // Controls if character can play MOVE_RIGHT animations
    m_bPassedMoveAt(false)
    , // true if NPC has gone thru the move at
    m_bExactMove(false)
    , // Use for scripted stuff - pixel perfect if true!
    m_bExactLook(false)
    , // Use for scripted stuff - pixel perfect if true!
    m_bExactLookComplete(false)
    , // true if exact look at is complete
    m_bUseAimMoveStyles(false)
    , // Should we use aimmovestyles?
    m_bMoveAtSnap(false)
    , // true if NPC should snap if passing move at
    m_bExactMoveBlending(false)
    , // true if blending from MOVE->IDLE with exact move on
    m_bAdvancePhysics(true)
    , // true if physics should be advanced
    m_bGhostMode(false)
    , // true if loco is being used for a net ghost character
    m_bGhostIsMoving(false)
    , // true if ghost should be moving ie. not idle
    m_bFrameMatchMoveAnim(false)
    , // Used for smooth move style switching
    m_bDynamicLipSyncAnim(false)
    , // true if our lipsync anims switches masks depending upon loco state.
    m_bStateChangedThisTick(false)
    , // True if our state changed this tick.
    m_bFaceIdleEnabled(true)
    , // true if blinking etc is allowed

    m_AnimState(STATE_NULL)
    , // Current animation state
    m_PlayAnimFlags(0)
    , // Current play animation flags
    m_DeltaPosScale(1, 1, 1)
    , // Use to slide delta pos
    m_DeltaPos(0, 0, 0)
    ...
    m_MoveAt(0, 0, 0)
    , // Location to move to
    m_ExactMoveBlendPos(0, 0, 0)
    , // Position to blend from

    m_ArriveDistSqr(150 * 150)
    , // Min distance sqaured to get from move at

    m_HeadLookAt(0, 0, 0)
    , // Head location to look at
    m_BodyLookAt(0, 0, 0)
    , // Body location to look at

    m_Motion(MOTION_NULL)
    , // Current motion
    m_ToTransition(MOTION_NULL)
    , // Current transition motion
    ...
    , // Current move style
    m_BlendMoveStyle(MOVE_STYLE_NULL)
    , // Blend move style
    m_BlendMoveStyleAmount(0.0f)
    , // 0 = m_MoveStyle, 1 = m_BlendMoveStyle

    m_CinemaRelativePos(0, 0, 0)
    , // Cinema relative position
    m_CinemaRelativeYaw(R_0)
    , // Cinema relative yaw

    m_CoverRelativePos(0, 0, 0)
    , // Cover relative position
    m_CoverRelativeYaw(R_0)
    , // Cover relative yaw

    m_FaceIdleTimer(0.0f)
    , // Time before next face idle (blink etc)
    m_FaceIdleMinInterval(2.0f)
    ,                           // Min time before next face idle
...
void loco::OnAdvance(float DeltaTime)
{
...
*/
    ...
    // Sanity check
    assert(abs(m_DeltaPos.GetX()) < (100.0f * 10.0f));
    assert(abs(m_DeltaPos.GetY()) < (100.0f * 10.0f));
    assert(abs(m_DeltaPos.GetZ()) < (100.0f * 10.0f));
    ...
    if ((m_Player.IsBlending() == false) && (CurrAnim.IsCinemaRelativeMode() || CurrAnim.IsCoverRelativeMode()) && (CurrAnim.GetAnimIndex() != -1)) {
        ...
        // Lookup anim info
        float           Frame = CurrAnim.GetFrame();
        const AnimInfo& ai = CurrAnim.GetAnimInfo();

        // Get current key
        AnimKey CurrKey;
        CurrAnim.GetMotionInterpKey(ai, Frame, CurrKey);
    ...
    float T = m_MoveAt.GetClosestPToLSegRatio(OldPos, NewPos);
    if ((T > 0) && (T < 1)) {
    ...
    if ((m_bExactMove) && (m_AnimState == STATE_IDLE)) {
        ...
        float Blend = m_Player.GetBlendAmount();

        // Blend to exact position?
        if ((m_bExactMove) && (m_bExactMoveBlending)) {
            // Compute blended XZ position (0 = m_ExactMoveBlendPos, 1 = m_MoveAt)
            NewPos.x = m_ExactMoveBlendPos.GetX() + (Blend * (m_MoveAt.GetX() - m_ExactMoveBlendPos.GetX()));
            NewPos.z = m_ExactMoveBlendPos.GetZ() + (Blend * (m_MoveAt.GetZ() - m_ExactMoveBlendPos.GetZ()));
    ...
    if ((GetState() == loco::STATE_IDLE) && (m_bExactLook) && (m_bExactLookComplete == false) && (abs(OldH) < R_90) && (abs(NewH) < R_90) && (x_sign(OldH) != x_sign(NewH))) {
...
void loco::UpdateFaceIdle(float DeltaTime)
{
    // Skip if dead
    if (m_bDead) {
        return;
...
void loco::UpdateEyeTracking(float DeltaTime)
{
    /*
    ...
    float Weight = m_EyeController.GetWeight();
    float Blend = DeltaTime * 2.0f;

    // Fade out if dead, playing a cinema or lip sync
    if ((m_bDead) || (m_Player.GetCurrAnim().GetAnimFlags() & loco::ANIM_FLAG_CINEMA) || (m_LipSyncController.IsPlaying())) {
        // Blend out
        Weight = std::max(0.0f, Weight - Blend);
    } else {
        // Blend in
        Weight = std::min(1.0f, Weight + Blend);
    }
...
int loco::GetMoveStyleAnimIndex(loco::move_style_anim MoveStyleAnim)
{
...
int loco::GetAnimIndex(loco::anim_type AnimType)
{
    // NULL?
    ...
    int Index = m_AnimLookupTable.m_Index[AnimType];
...
bool loco::SetState(loco::state State)
{
    // Anims not ready?
    if (!IsAnimLoaded()) {
        return false;
...
const char* loco::GetStateName(loco::state State) const
{
    ...
    case STATE_MOVE:
        return "STATE_MOVE";
    case STATE_PLAY_ANIM:
        return "STATE_PLAY_ANIM";
    }
}
...
void loco::SetStateAnimRate(loco::state State, float Rate)
{
    assert(Rate >= 0);
    assert(State >= 0);
...
float loco::GetStateAnimRate(loco::state State) const
{
    assert(State >= 0);
    assert(State < STATE_TOTAL);
...
bool loco::PlayMaskedAnim(const char* pAnimGroup, const char* pAnim, float BlendTime, uint32_t Flags)
{
    // Lookup controller
    ...
    // Lookup the anim
    int iAnim = Cont.GetAnimIndex(pAnim);
    if (iAnim == -1) {
        return false;
    }

    // Lookup blend time?
    const AnimInfo& ai = Cont.GetAnimInfo(iAnim);
    if (ai.GetBlendTime() != -1.0f) {
        BlendTime = ai.GetBlendTime();
    }
...
bool loco::PlayMaskedAnim(int iAnim, bone_masks_type MaskType, float BoneMaskBlendTime, uint32_t Flags)
{
    // No anim?
    if (iAnim == -1) {
        return false;
...
bool loco::PlayMaskedAnim(loco::anim_type AnimType, bone_masks_type MaskType, float BoneMaskBlendTime, uint32_t Flags)
{
    // If no anims are loaded, then quit now...
    if (!IsAnimLoaded()) {
        return false;
    ...
    assert(AnimType >= 0);
    assert(AnimType < loco::ANIM_TOTAL);
    int iAnim = GetAnimIndex(AnimType);
...
bool loco::PlayAdditiveAnim(int iAnim, float BlendInTime, float BlendOutTime, uint32_t Flags)
{
    // No anim found?
    if (iAnim == -1) {
        return false;
...
bool loco::PlayAdditiveAnim(loco::anim_type AnimType, float BlendInTime, float BlendOutTime, uint32_t Flags)
{
    // If no anims are loaded, then quit now...
    if (!IsAnimLoaded()) {
        return false;
...
bool loco::PlayAdditiveAnim(const char* pAnim, float BlendInTime, float BlendOutTime, uint32_t Flags)
{
    // Lookup anim group data
    AnimGroup* pAnimGroup = (AnimGroup*)m_hAnimGroup.getPointer();
    if (!pAnimGroup) {
        return false;
    }

    // Lookup animation index
    int iAnim = pAnimGroup->GetAnimIndex(pAnim);
    if (iAnim == -1) {
        return false;
    }
...
bool loco::PlayLipSyncAnim(const AnimGroup::handle& hAnimGroup, int iAnim, uint32_t VoiceID, uint32_t Flags)
{
    m_bDynamicLipSyncAnim = false;
...
/*
    ...
    } else if (Flags & ANIM_FLAG_MASK_TYPE_UPPER_BODY) {                              // Upper body?
        Controller.SetBoneMasks(GetBoneMasks(loco::BONE_MASKS_TYPE_UPPER_BODY), 0.5f);
    } else if (Flags & ANIM_FLAG_MASK_TYPE_FACE) { // Face?
        Controller.SetBoneMasks(GetBoneMasks(loco::BONE_MASKS_TYPE_FACE), 0.5f);
    } else if (Flags & ANIM_FLAG_MASK_TYPE_DYNAMIC) // Face?
    {
...
bool loco::PlayLipSyncAnim(const AnimGroup::handle& hAnimGroup, const char* pAnimName, uint32_t VoiceID, uint32_t Flags)
{
    // Lookup anim group data
    AnimGroup* pAnimGroup = hAnimGroup.getPointer();
    if (!pAnimGroup) {
        return false;
    }
...
bool loco::PlayLipSyncAnim(const AnimGroup::handle& hAnimGroup, const char* pAnimName, const char* pAudioName, uint32_t Flags)
{
    /*
    // Try start the sound if not in the artist viewer (lip sync controller will start it when event is found)
    uint32_t VoiceID = g_AudioMgr.Play(pAudioName, (Flags & loco::ANIM_FLAG_ARTIST_VIEWER) == 0); // Name, AutoStart
...
bool loco::PlayLipSyncAnim(int iAnim, const char* pAudioName, uint32_t Flags)
{
    /*
    // Try start the sound if not in the artist viewer (lip sync controller will start it when event is found)
    uint32_t VoiceID = g_AudioMgr.Play(pAudioName, (Flags & loco::ANIM_FLAG_ARTIST_VIEWER) == 0); // Name, AutoStart
...
bool loco::PlayLipSyncAnim(loco::anim_type AnimType, const char* pAudioName, uint32_t Flags)
{
    // Lookup animation index
    int iAnim = GetAnimIndex(AnimType);
    if (iAnim == -1) {
    ...
    // Try start the sound if not in the artist viewer (lip sync controller will start it when event is found)
    uint32_t VoiceID = 0; //g_AudioMgr.Play(pAudioName, (Flags & loco::ANIM_FLAG_ARTIST_VIEWER) == 0); // Name, AutoStart
...
void loco::InitBoneMasks(const Geom* pGeom)
{
    ...
    struct init_bone_mask
    {
        ...
        Geom::BoneMask*             m_pDefault; // Default geom bone masks
    };
    ...
    s_OneBoneMasks.numBones = (pGeom) ? pGeom->numBones : 0;

    const int Count = (sizeof(s_InitBoneMaskList) / sizeof(s_InitBoneMaskList[0]));
...
const Geom::BoneMask& loco::GetBoneMasks(loco::bone_masks_type Type)
{
    // Must be valid type
    assert(Type >= 0);
...
void loco::InitProperties(const Geom* pGeom)
{
    ...
    const Geom::PropertySection* pSection = pGeom->FindPropertySection("AIMER");
    if (pSection) {
    ...
    for (int i = 0; i < loco::MOVE_STYLE_COUNT; i++) {
        // Lookup info
...
struct play_anim
{
    AnimGroup::handle hAnimGroup;   // Anim group handle
    int               iAnim;        // Index of anim
    loco::anim_type   AnimType;     // Loco anim type
    const char*       pAnimName;    // Anim name
    const char*       pAudioName;   // Audio to play
    int               VoiceID;      // Voice ID of audio
    float             BlendInTime;  // Blend in time
    float             BlendOutTime; // Blend out time
    uint32_t          Flags;        // Flags
    float             PlayTime;     // Play time
    float             StartFrame;   // Frame to start on
};

//==============================================================================

bool loco::PlayAnim(const AnimGroup::handle& hAnimGroup, int iAnim, float BlendTime, uint32_t Flags, float PlayTime)
{
    // If no anims are loaded, then quit now...
    if (!hAnimGroup.isLoaded()) {
        return false;
    ...
    if ((Flags & ANIM_FLAG_RESTART_IF_SAME_ANIM) == 0) {
        ...
        if ((CurrAnim.GetAnimGroupHandle().getPointer() == hAnimGroup.getPointer()) && (CurrAnim.GetAnimTypeIndex() == iAnim)) {
            return false;
        }
    }
...
bool loco::PlayAnim(const AnimGroup::handle& hAnimGroup, const char* pAnim, float BlendTime, uint32_t Flags, float PlayTime)
{
    // Lookup anim group data
    AnimGroup* pAnimGroup = hAnimGroup.getPointer();
    if (!pAnimGroup) {
...
bool loco::PlayAnim(const char* pAnimGroup, const char* pAnim, float BlendTime, uint32_t Flags, float PlayTime)
{
    // Play a masked anim?
    if ((Flags & loco::ANIM_FLAG_MASK_TYPE_UPPER_BODY) || (Flags & loco::ANIM_FLAG_MASK_TYPE_FACE)) {
        // Play masked anim
...
bool loco::PlayAnim(const char* pAnim, float BlendTime, uint32_t Flags, float PlayTime)
{
    // If no anims are loaded, then quit now...
    if (!IsAnimLoaded()) {
        return false;
    ...
    // Lookup animation index
    int iAnim = m_Player.GetAnimIndex(pAnim);
...
bool loco::PlayAnim(loco::anim_type AnimType, float BlendTime, uint32_t Flags, float PlayTime)
{
    // If no anims are loaded, then quit now...
    if (!IsAnimLoaded()) {
        return false;
...
bool loco::PlayDeathAnim(const AnimGroup::handle& hAnimGroup, const char* pAnim, float BlendTime, uint32_t Flags)
{
...
bool loco::PlayDeathAnim(loco::anim_type AnimType, float BlendTime, uint32_t Flags)
{
...
bool loco::IsPlayAnimComplete(void) const
{
...
loco_motion_controller& loco::GetMotionController(uint32_t AnimFlags)
{
    // Current anim?
...
//=========================================================================
/*
loco_additive_controller& loco::GetAdditiveController(uint32_t AnimFlags)
{
    // Lookup controller index
    int iController = 0; // Default if no flags
...
*/
bool loco::IsAtPosition(const Vector3& Pos, float ArriveDistSqr)
{
...
void loco::SetPosition(const Vector3& Position)
{
    ...
    if (m_bGhostMode) {
        ...
        if (SpeedSqr > 0.000001f) {
            ...
            float Offset = 100.0f / sqrt(SpeedSqr);
...
Matrix4 loco::GetWeaponL2W(int iHand)
{
    // Must be in range!
    assert(iHand >= 0);
    assert(iHand <= 1);
...
static float FLAG_BIND_ROT_TWIST = 30.0f;
static float FLAG_BIND_ROT_PITCH = 0.0f;
static float FLAG_BIND_ROT_YAW = 0.0f;
static float FLAG_BIND_ROT_ROLL = 0.0f;
static float FLAG_BIND_POS_X = 0.0f;
static float FLAG_BIND_POS_Y = 50.0f;
static float FLAG_BIND_POS_Z = 16.385f;
...
void loco::SetCinemaRelativeMode(bool bEnable)
{
...
void loco::SetCoverRelativeMode(bool bEnable)
{
    // Set relative mode on all controllers
...
void loco::SetHeadLookAt(const Vector3& Target, bool bSetEyesLookAt /*= true */)
{
    // Must be valid!
    assert(Target.IsValid());
...
void loco::SetLookAt(const Vector3& Target, bool bSetEyesLookAt /*= true */)
{
    // Set head look at
...
void loco::ComputeMotion(bool          bAllowDir[4], // F L B R
                         Radian        LookYaw,
                         Radian        MoveYaw,
                         loco::motion& Motion,
                         Radian&       DeltaYaw)
{
    ...
    if (m_Player.IsBlending()) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            if ((bAllowDir[TestMotion] == false) && (bAllowDir[(TestMotion + 2) & 3] == false)) {
                continue;
            }
...
void loco::ComputeValidMotion(loco::motion& Motion, Radian& DeltaYaw)
{
    ...
    if ((m_Motion >= loco::MOTION_FRONT) && (m_Motion <= loco::MOTION_RIGHT) && (bAllowDir[m_Motion] == false)) {
        // Clear
    ...
    if ((abs(MoveDeltaYaw) > R_45) && (abs(LookDeltaYaw) > R_90)) {
    ...
    if ((abs(DeltaYaw) > R_90) && (Motion == ((m_Motion + 2) & 3))) {
...
loco::move_style_anim loco::GetCurrentMoveStyleAnim(void)
{
    ...
    int iAnim = m_Player.GetCurrAnim().GetAnimTypeIndex();

    // Using main package or override?
    int16_t* AnimIndices = &m_AnimLookupTable.m_Index[m_MoveStyle * MOVE_STYLE_ANIM_COUNT];
...
int loco::GetMoveStyleAnimIndex(move_style Style, move_style_anim Anim)
{
    // Invalid move style?
    ...
    int16_t* AnimIndices = &m_AnimLookupTable.m_Index[Style * MOVE_STYLE_ANIM_COUNT];
...
bool loco::IsValidMoveStyle(move_style Style)
{
    // Invalid move style?
    if ((Style <= MOVE_STYLE_NULL) || (Style >= MOVE_STYLE_COUNT)) {
        //ASSERTS(0, "You passed in an invalid style!");
    ...
    if ((AnimIndices[MOVE_STYLE_ANIM_IDLE] != -1) && (AnimIndices[MOVE_STYLE_ANIM_IDLE_TURN_LEFT] != -1) && (AnimIndices[MOVE_STYLE_ANIM_IDLE_TURN_RIGHT] != -1) && (AnimIndices[MOVE_STYLE_ANIM_MOVE_FRONT] != -1)) {
        return true;
    } else {
        return false;
    }
...
void loco::SwitchMoveStyleSmoothly(move_style Style, move_style_anim MoveStyleAnim)
{
    ...
    float OldFrame = m_Player.GetCurrAnim().GetFrameParametric();

    // Setup the new move style
    SetMoveStyle(Style);

    // Lookup new animation
    int iNewAnim = m_MoveStyleInfo.m_iAnims[MoveStyleAnim];
    if (iNewAnim != -1) {
...
inline float Interp(float T0, float T1, float T)
{
...
void loco::UpdateMoveStyle(void)
{
    ...
    int iMixAnim = -1;
    if ((m_BlendMoveStyle != MOVE_STYLE_NULL) && (m_BlendMoveStyle != m_MoveStyle) && (m_BlendMoveStyleAmount != 0.0f)) {
...
void loco::SetMoveStyle(loco::move_style InputStyle)
{
    ...
    m_bFrameMatchMoveAnim = (Style != m_MoveStyle) && (GetState() == loco::STATE_MOVE) && (m_Motion >= MOTION_FRONT) && (m_Motion <= MOTION_RIGHT);

    // Set default anim group and animation indices
    int16_t* pIndices = &m_AnimLookupTable.m_Index[Style * MOVE_STYLE_ANIM_COUNT];
    m_MoveStyleInfo.m_hAnimGroup = m_hAnimGroup;
    assert(sizeof(m_MoveStyleInfo.m_iAnims[0]) == sizeof(m_AnimLookupTable.m_Index[0]));
    memcpy(m_MoveStyleInfo.m_iAnims, pIndices, sizeof(m_MoveStyleInfo.m_iAnims));
    ...
    if (m_Player.GetBlendAnim().GetAnimIndex() == -1) {
        int iIdle = m_MoveStyleInfo.m_iAnims[MOVE_STYLE_ANIM_IDLE];
        if (iIdle != -1) {
...
void loco::SetBlendMoveStyleAmount(float Amount)
{
    // Update move style info?
    if ((m_BlendMoveStyle != MOVE_STYLE_NULL) && (m_BlendMoveStyle != m_MoveStyle) && (m_BlendMoveStyleAmount != Amount)) {
        // Keep new mix amount
...
void loco::SetAimerBlendSpeed(float AimerBlendSpeed)
{
    // Scale by property
    if (m_pGeom) {
        float PropertyBlendSpeed = 1.0f;

        // TO DO: Store this "m_PropertyBlendSpeed" in the init function
        const Geom::PropertySection* pSection = m_pGeom->FindPropertySection("AIMER");
        m_pGeom->GetPropertyFloat(pSection, "BlendSpeed", &PropertyBlendSpeed);
        AimerBlendSpeed *= PropertyBlendSpeed;
    }

    m_MoveStyleInfo.m_AimerBlendSpeed = std::max(0.1f, AimerBlendSpeed);
}

//=========================================================================

void loco::SetAimerWeight(float Weight, float BlendTime)
{
...
void loco::SetGhostMode(bool bEnable)
{
    // Store
...
{
    float D = (P4.GetX() - P3.GetX()) * (P1.GetZ() - P3.GetZ()) - (P4.GetZ() - P3.GetZ()) * (P1.GetX() - P3.GetX());
    if (D == 0) {
        return Vector3(0, 0, 0);
    }
    float Q = (P4.GetZ() - P3.GetZ()) * (P2.GetX() - P1.GetX()) - (P4.GetX() - P3.GetX()) * (P2.GetZ() - P1.GetZ());
...
void loco::SetAllowMotion(loco::motion Motion, bool bEnable)
{
    switch (Motion) {
    case loco::MOTION_FRONT:
        m_bAllowFrontMotion = bEnable;
        break;
    case loco::MOTION_LEFT:
        m_bAllowLeftMotion = bEnable;
        break;
    case loco::MOTION_BACK:
        m_bAllowBackMotion = bEnable;
        break;
    case loco::MOTION_RIGHT:
        m_bAllowRightMotion = bEnable;
        break;
...
bool loco::IsMotionAllowed(loco::motion Motion)
{
    switch (Motion) {
    case loco::MOTION_FRONT:
        return m_bAllowFrontMotion;
    case loco::MOTION_LEFT:
        return m_bAllowLeftMotion;
    case loco::MOTION_BACK:
        return m_bAllowBackMotion;
    case loco::MOTION_RIGHT:
        return m_bAllowRightMotion;
...
void loco::OnEnumProp(prop_enum& List)
{
    ...
    for (int i = MOVE_STYLE_NULL + 1; i < MOVE_STYLE_COUNT; i++) {
        ...
        int iHeader = List.PushPath(xfs("LocoStyles\\%s\\", pName));
...
bool loco::OnProperty(prop_query& I)
{
    // Physics
    ...
    if (I.IsBasePath("LocoStyles")) {
        int iHeader = I.PushPath("LocoStyles\\");
        ...
        for (int i = MOVE_STYLE_NULL + 1; i < MOVE_STYLE_COUNT; i++) {
            ...
            int                      iStyleHeader = I.PushPath(GetMoveStyleHeader(i));
            move_style_info_default& Info = m_MoveStyleInfoDefault[i];

            // Anim package?
            if (I.IsVar("AnimPackage")) {
                if (I.IsRead()) {
                    I.SetVarExternal(Info.m_hAnimGroup.getName().c_str(), RESOURCE_NAME_SIZE);
                } else {
                    if (I.GetVarExternal()[0]) {
                        Info.m_hAnimGroup.setName(I.GetVarExternal());

                        // Lookup move style anims
                        const AnimGroup* pAnimGroup = Info.m_hAnimGroup.getPointer();
                        if (pAnimGroup) {
                            // Init anims
                            const char* pStyle = GetMoveStyleName(i);
                            for (int j = 0; j < MOVE_STYLE_ANIM_COUNT; j++) {
...
Vector3 loco::GetBonePosition(int Bone)
{
    const int nBones = m_Player.GetNActiveBones();
...
int loco::GetRandomBone(void)
{
    ...
    const int nBones = m_Player.GetNBones();

    if (nBones > 0) {
        while (true) {
            int Bone = x_irand(0, nBones);
            int i;
            for (i = 0; i < 4; ++i) {
                assert(m_Player.GetAnimGroup());
                if (strstr(m_Player.GetAnimGroup()->GetBone(Bone).name.c_str(), BoneNames[i])) {
                    return Bone;
                }
            }
        }
