...
void lore_object::OnAdvanceLogic(f32 DeltaTime)
{
    ...
    //
    {
        ...
        m_MaxAlpha = (s32)(255 * ((m_AnimState <= 0.3f) ? (1.0f) : (1.0f - ((m_AnimState - 0.3f) / 0.7f))));
    }
}
...
void lore_object::OnRenderTransparent(void)
{
    ...
    case SELECT_BRACKETS:
    {
        ...
        P[0].GetX() = BBox.Min.GetX();
        P[0].GetY() = BBox.Min.GetY();
        P[0].GetZ() = BBox.Min.GetZ();
        P[1].GetX() = BBox.Min.GetX();
        P[1].GetY() = BBox.Min.GetY();
        P[1].GetZ() = BBox.Max.GetZ();
        P[2].GetX() = BBox.Min.GetX();
        P[2].GetY() = BBox.Max.GetY();
        P[2].GetZ() = BBox.Min.GetZ();
        P[3].GetX() = BBox.Min.GetX();
        P[3].GetY() = BBox.Max.GetY();
        P[3].GetZ() = BBox.Max.GetZ();
        P[4].GetX() = BBox.Max.GetX();
        P[4].GetY() = BBox.Min.GetY();
        P[4].GetZ() = BBox.Min.GetZ();
        P[5].GetX() = BBox.Max.GetX();
        P[5].GetY() = BBox.Min.GetY();
        P[5].GetZ() = BBox.Max.GetZ();
        P[6].GetX() = BBox.Max.GetX();
        P[6].GetY() = BBox.Max.GetY();
        P[6].GetZ() = BBox.Min.GetZ();
        P[7].GetX() = BBox.Max.GetX();
        P[7].GetY() = BBox.Max.GetY();
        P[7].GetZ() = BBox.Max.GetZ();
...
xbool lore_object::OnProperty(prop_query& rPropQuery)
{
    ...
    if (rPropQuery.VarBool("Active", m_bActive)) {
        return TRUE;
    }
    if (rPropQuery.VarBool("UseGeomSize", m_bUseGeometrySize)) {
        return TRUE;
    }
    if (rPropQuery.VarInt("LoreID", m_LoreID)) {
        return TRUE;
    }

    if (rPropQuery.VarFloat("ViewDist", m_ViewDist)) {
        return TRUE;
    }
    if (rPropQuery.VarBBox("Size", m_FocusBox)) {
        return TRUE;
    }

    if (rPropQuery.VarFloat("TextDist", m_TextDist)) {
        return TRUE;
    }
    if (rPropQuery.VarFloat("ScanDist", m_ScanDist)) {
        return TRUE;
    }
    if (rPropQuery.VarBool("ScanLOS", m_bScanLOS)) {
        return TRUE;
    }

    if (rPropQuery.VarGUID("TriggerGuid", m_TriggerGuid)) {
        return TRUE;
    }
    if (rPropQuery.VarGUID("SpatialGuid", m_SpatialGuid)) {
        return TRUE;
    }

    if (rPropQuery.VarBool("TurnOff", m_bAutoOff)) {
        return TRUE;
    }
    if (rPropQuery.VarBool("Destroy", m_bDestroyAfterAcquired)) {
        return TRUE;
    }
    if (rPropQuery.VarBool("TriggerOnRestore", m_bActivateTriggerOnRestore)) {
        return TRUE;
    }
