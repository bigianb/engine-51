...
void corpse_pain::Setup(const pain& Pain, corpse& Corpse)
{
    ...
    // Lookup pain info
    float               ForceFarDist = 100.0f * 10.0f;
    bool                bSplash = false;
    ...
    if (CorpsePain.HasCollision()) {
        ...
        if (Collision.ObjectHitGuid == Corpse.GetGuid()) {
            ...
            object_ptr<actor> pActor(Collision.ObjectHitGuid, Corpse.getObjectManager());
            if (pActor) {
                bOnDeath = true;
                ...
                if (pGeom) {
                    ...
                    int iBone = Collision.PrimitiveKey & 0xFF;
                    if ((iBone >= 0) && (iBone < pGeom->numBones)) {
                        iHitRigidBody = pGeom->bones[iBone].rigidBodyIdx;
                    }
                }
            }
        }
    ...
    m_OriginNetSlot = -1;
    object_ptr<actor> pOriginActor(CorpsePain.GetOriginGuid(), Corpse.getObjectManager());
    if (pOriginActor) {
        m_OriginNetSlot = pOriginActor->net_GetSlot();
    }

    // Use the hit_type to determine how to react
    int HitType = CorpsePain.GetHitType();
    if (HitType == 4) // Melee?
    {
        // Setup melee pain
        m_Type = TYPE_MELEE;
...
volatile float DEATH_FORCE_SCALE = 3.0f;
...
void corpse_pain::Apply(corpse& Corpse)
{
    ...
    IJB physics_inst& PhysicsInst = Corpse.GetPhysicsInst();
    ...
    case TYPE_MELEE:
        {
            // Lookup tweaks
            float ForceScale = GetTweakF32( "DEADBODY_MeleeForceScale", 2000.0f );
    ...
    case TYPE_SPLASH:
        {
            // Lookup tweaks
            float ForceScale = GetTweakF32( "DEADBODY_ExplosionForceScale", 2000.0f );
            float DirAmount  = GetTweakF32( "DEADBODY_ExplosionDirAmount", 0.25f );
            float UpAmount   = GetTweakF32( "DEADBODY_ExplosionUpAmount", 0.75f );
    ...
    case TYPE_RIGID_BODY:
        {
            // Lookup tweaks
            float ForceScale = GetTweakF32( "DEADBODY_ImpactForceScale", 2000.0f );
    ...
    case TYPE_BLAST:
        {
            // Lookup tweaks
            float ForceScale = GetTweakF32( "DEADBODY_ImpactForceScale", 2000.0f );
            float Radius     = GetTweakF32( "DEADBODY_ImpactRadius",     600.0f );
...
guid corpse_pain::GetOriginGuid() const
{
...
const Vector3& corpse_pain::GetDirection() const
{
    return m_Direction;
}
...
bool corpse_pain::IsDirectHit() const
{
    return m_bDirectHit;
}
