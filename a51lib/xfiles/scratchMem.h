...
#define ALIGN_16(n)     ( (((int)(n)) +  15) & ( -16) )
#define SMEM_ALIGN      ALIGN_16

void    smem_ChangeSize         ( int NBytes );
uint8_t*   smem_BufferAlloc        ( int NBytes ); // Can return NULL
uint8_t*   smem_StackAlloc         ( int NBytes ); // Can return NULL
void    smem_StackPushMarker    ( void );
void    smem_StackPopToMarker   ( void );
int     smem_GetMaxUsed         ( void );
...
int     smem_GetActiveID        ( void ) ;
uint8_t*   smem_GetActiveStartAddr ( void ) ;
uint8_t*   smem_GetActiveEndAddr   ( void ) ;
